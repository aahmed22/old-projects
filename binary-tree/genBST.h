...
public:
	...
	// Three additional functions: nodeCount(), leaveCount(), mirrorImage(), included in genBST.h
...
protected:
	...
	int nodeCount(BSTNode<T>*); // Function prototype for nodeCount.
	int leaveCount(BSTNode<T>*); // Function prototype for leaveCount.
	void mirrorImage(BSTNode<T>*); // Function prototype for mirrorImage.
...
template<class T>
int BST<T>::leaveCount(BSTNode<T> *p)
{
    int tmp = 0;
    ...
    {
       if(p->left != NULL)
             tmp += leaveCount(p->left);
       if(p->right != NULL)
             tmp += leaveCount(p->right);
    }
